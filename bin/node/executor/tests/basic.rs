...
fn changes_trie_block() -> (Vec<u8>, Hash) {
	construct_block(
		...
		vec![
			...
			CheckedExtrinsic {
				signed: Some((alice(), signed_extra(0, 0))),
				function: Call::Balances(pallet_balances::Call::transfer(bob().into(), 69 * CHI)),
			},
		]
	)
}
	...
	let block2 = construct_block(
		...
		vec![
			...
			CheckedExtrinsic {
				...
				function: Call::Balances(pallet_balances::Call::transfer(alice().into(), 5 * CHI)),
			},
			CheckedExtrinsic {
				signed: Some((alice(), signed_extra(1, 0))),
				function: Call::Balances(pallet_balances::Call::transfer(bob().into(), 15 * CHI)),
			}
		]
	);
...
fn successful_execution_with_native_equivalent_code_gives_ok() {
	let mut t = TestExternalities::<BlakeTwo256>::new_with_code(COMPACT_CODE, Storage {
		top: map![
			<frame_system::Account<Runtime>>::hashed_key_for(alice()) => {
				(0u32, 0u8, 111 * CHI, 0u128, 0u128, 0u128).encode()
			},
			<pallet_balances::TotalIssuance<Runtime>>::hashed_key().to_vec() => {
				(111 * CHI).encode()
			},
	...
	t.execute_with(|| {
		...
		assert_eq!(Balances::total_balance(&alice()), 42 * CHI - fees);
		assert_eq!(Balances::total_balance(&bob()), 69 * CHI);
	});
}
...
fn full_native_block_import_works() {
	...
	t.execute_with(|| {
		let fees = transfer_fee(&xt(), fm);
		assert_eq!(Balances::total_balance(&alice()), 42 * CHI - fees);
		assert_eq!(Balances::total_balance(&bob()), 169 * CHI);
	...
	t.execute_with(|| {
		...
		assert_eq!(
			...
			alice_last_known_balance - 10 * CHI - fees,
		);
		assert_eq!(
...
fn full_wasm_block_import_works() {
	...
	t.execute_with(|| {
		assert_eq!(Balances::total_balance(&alice()), 42 * CHI - transfer_fee(&xt(), fm));
		assert_eq!(Balances::total_balance(&bob()), 169 * CHI);
		alice_last_known_balance = Balances::total_balance(&alice());
	});
	...
	t.execute_with(|| {
		assert_eq!(
			Balances::total_balance(&alice()),
			alice_last_known_balance - 10 * CHI - transfer_fee(&xt(), fm),
		);
		assert_eq!(
			Balances::total_balance(&bob()),
			179 * CHI - 1 * transfer_fee(&xt(), fm),
		);
	});
}
	...
	let b = construct_block(
		...
		vec![
			...
			CheckedExtrinsic {
				...
				function: Call::Contracts(
					pallet_contracts::Call::instantiate::<Runtime>(1 * CHI, 10_000, transfer_ch, Vec::new())
				),
			},
			CheckedExtrinsic {
