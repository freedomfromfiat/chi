...
impl<T: Trait> Module<T> {
	...
	fn fetch_price_and_send_signed() -> Result<(), &'static str> {
		...
		for (acc, res) in &results {
			match res {
				Ok(()) => debug::info!("[{:?}] Submitted price of {} MILLICHI", acc.id, price),
	...
	/// Fetch current price and return the result in MILLICHI.
	fn fetch_price() -> Result<u32, http::Error> {
		...
		debug::warn!("Got price: {} MILLICHI", price);
	...
	///
	/// Returns `None` when parsing failed or `Some(price in MILLICHI)` when parsing is successful.
